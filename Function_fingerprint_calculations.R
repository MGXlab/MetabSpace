...
##------After function calculations with results----------
# results <- Fingerprint_calc(compoundslist)
# write_delim(results, "calcul_fp_20211005.csv", delim = ";")
# 
# 
# fp_allcal <- as.data.frame(colnames(results))
# names(fp_allcal) <- "colnam"
# 
# setwd("C:/Users/pipe0902/Kruvelab/Anneli Kruve - Toxicity project data/data_csv for coding/SIRIUS_MassBank")
# 
# fp_common <- read_delim("common_fp_list.txt", delim = ";") %>%
#   select(absoluteIndex) %>%
#   mutate(colnam = paste("Un",absoluteIndex, sep = "")) %>%
#   inner_join(fp_allcal)
# 
# fp_common <- as.vector(fp_common$colnam)
# 
# 
# final_fingerpr <- results %>%
#   select(dtxsid, all_of(fp_common))


SMILESFingerprints_mod_woMass <- function(compoundslistwithSMILES){
  
  #substructure fingerprints
  row <- c(1:307) #first one has mass, 307 fingerprints actually
  list <- as.data.frame(row)
  
  col_names_substr <- c(paste("Un", c(55:361), sep = ""))
